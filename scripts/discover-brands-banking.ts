/**
 * Discover brands for Financial Services > Banking ONLY
 *
 * This script discovers brands for all Banking sub-subcategories:
...
async function discoverBrandsForBanking() {
  console.log("🚀 Brand Discovery Script for Financial Services > Banking\n");
  console.log("⚠️  WARNING: This script will make OpenAI API calls which cost money!\n");

  // Step 1: Get all Banking sub-subcategories
  console.log("📋 Fetching Banking taxonomies...");
  const bankingTaxonomies = await prisma.businessTaxonomy.findMany({
    where: {
  ...
  console.log(`✅ Found ${bankingTaxonomies.length} Banking sub-subcategories:`);
  bankingTaxonomies.forEach((t, i) => {
    console.log(`   ${i + 1}. ${t.sub_subcategory}`);
  });
  console.log();

  // Calculate estimated cost
  const totalTaxonomies = bankingTaxonomies.length;
  const brandsPerTaxonomy = BRANDS_PER_STAGE * STAGES.length; // 10 × 3 = 30
  const totalBrands = totalTaxonomies * brandsPerTaxonomy; // 4 × 30 = 120
  ...
  // Process each Banking sub-subcategory
  for (let i = 0; i < bankingTaxonomies.length; i++) {
    const taxonomy = bankingTaxonomies[i];

    console.log(`${"=".repeat(80)}`);
    console.log(`[${i + 1}/${bankingTaxonomies.length}] Processing: ${taxonomy.sub_subcategory}`);
    console.log(
...
discoverBrandsForBanking().catch((error) => {
