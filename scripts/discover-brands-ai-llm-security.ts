...
async function discoverBrandsForAILLMSecurity() {
  console.log("🚀 Starting automated brand discovery for Technology > AI & LLM Security\n");
  ...
  // Step 2: Find AI & LLM Security subcategory
  const aiLLMSecuritySubcategory = await prisma.businessTaxonomy.findFirst({
    where: {
      category: "Technology",
  ...
  if (!aiLLMSecuritySubcategory) {
    throw new Error("AI & LLM Security subcategory not found");
  }

  console.log(`✅ Found subcategory: ${aiLLMSecuritySubcategory.subcategory}\n`);

  // Step 3: Get all sub-subcategories under AI & LLM Security
  ...
  if (subSubcategories.length === 0) {
    console.log("⚠️  No sub-subcategories found under AI & LLM Security");
    return;
  }
  ...
  // Step 4: Process each sub-subcategory
  for (let i = 0; i < subSubcategories.length; i++) {
    ...
    if (i < subSubcategories.length - 1) {
  ...
  console.log("📊 SUMMARY");
  console.log(`${"=".repeat(80)}`);
  console.log(`Total sub-subcategories processed: ${subSubcategories.length}`);
...
discoverBrandsForAILLMSecurity()
  .then(async () => {
