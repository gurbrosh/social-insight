...
/**
 * GET: Export this project's BlogNewsAnalysis records as CSV.
 */
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const filename = `BlogNewsAnalysis-${projectId.slice(-8)}-${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    console.error("[blog-analysis/export]", err);
